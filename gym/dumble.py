...
class angleFinder:
    def __init__(self, lmlist, p1, p2, p3, p4, p5, p6, drawPoints=False):
        self.lmlist = lmlist
        self.p1 = p1
        self.p2 = p2
        self.p3 = p3
    ...
    # finding angles
    def angle(self):
        if len(self.lmlist) >= max(self.p1, self.p2, self.p3, self.p4, self.p5, self.p6):
...
while True:
    ...
    hands = angle1.angle()
    if hands is not None:  # Ensure hands angles are not None
        left, right = hands[0:]
