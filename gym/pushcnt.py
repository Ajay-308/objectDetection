...
cap = cv2.VideoCapture(0)  # Use the default camera (laptop camera)
...
def angles(lmlist, p1, p2, p3, p4, p5, p6, drawpoints):
    ...
    if len(lmlist) != 0:
        ...
        # Check if point1 contains enough values
        if len(point1) < 2:
            print("Not enough values in point1:", point1)
            return

        # Unpack x and y coordinates from point1
        x1, y1 = point1[1:-1]
        point2 = lmlist[p2]
        point3 = lmlist[p3]
        point4 = lmlist[p4]
